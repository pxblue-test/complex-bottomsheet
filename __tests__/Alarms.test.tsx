...
describe('bottom sheet', function () {
  ...
  it('filters events correctly', () => {
      ...
      // alarms
      wrapper.instance().setState({showAlarms: true, showActiveAlarms: true, showEvents: false, showSessions: false});
      sorted = wrapper.instance().filteredEvents(wrapper.instance().sortedEvents());
      expect(sorted).toEqual(alarms);

      // sessions
      wrapper.instance().setState({showAlarms: false, showActiveAlarms: false, showEvents: false, showSessions: true});
      sorted = wrapper.instance().filteredEvents(wrapper.instance().sortedEvents());
      expect(sorted).toEqual(sessions);

      // settings and active
      wrapper.instance().setState({showAlarms: false, showActiveAlarms: true, showEvents: true, showSessions: false});
      sorted = wrapper.instance().filteredEvents(wrapper.instance().sortedEvents());
